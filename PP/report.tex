...
%Classical Paxos (the first version of Paxos) was first published by Leslie
%Lamport in 1998\cite{classic-paxos}. Since then many variations of the algorithm
%emerged\footnote{Just a little bit of
%    \href{http://scholar.google.com/scholar?q=paxos}{searching} will give dozens
%    of algorithms that have a word "Paxos" in it.}.
...
Generalized Consensus and Paxos generalizes state-machine replication by reaching
...
Windows\footnote{\url{http://www.erlang.org/doc/installation\_guide/INSTALL.html}}.
...
system were necessary to compile newer version of Erlang (R15B02) on the version
3 of Tilera MDE. They have been submitted upstream.
...
However, logarithmic speedup is observed with a cap of ~12, like observed from
figure~\ref{fig:parallel_speedup}.

\begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth]{images/parallel_speedup.pdf}
...
notice the usability change; however, this would potentially eliminate shared
memory bottleneck.
